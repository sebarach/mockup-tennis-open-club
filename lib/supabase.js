...
export const playersAPI = {
  ...
  async create(playerData, avatarFile = null) {
    ...
    if (avatarFile && data.id) {
      try {
        console.log('🔄 Subiendo avatar para jugador:', data.id, data.name)
        const uploadResult = await storageAPI.uploadAvatar(avatarFile, data.id)
        console.log('✅ Avatar subido exitosamente:', uploadResult)
        ...
        if (updateError) {
          ...
          console.log('✅ Avatar actualizado en BD correctamente')
  ...
  async update(id, playerData, avatarFile = null) {
    ...
    if (avatarFile) {
      try {
        console.log('🔄 Actualizando avatar para jugador:', id)
        // Eliminar avatar anterior si existe
        if (currentPlayer.avatar_path) {
          console.log('🗑️ Eliminando avatar anterior:', currentPlayer.avatar_path)
          await storageAPI.deleteAvatar(currentPlayer.avatar_path)
        }

        // Subir nuevo avatar
        const uploadResult = await storageAPI.uploadAvatar(avatarFile, id)
        console.log('✅ Nuevo avatar subido:', uploadResult)
